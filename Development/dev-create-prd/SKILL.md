...
If no source is provided, ask the user to specify a source path before proceeding.
...
The PRD is a **standalone HTML document**. The structure below is also a machine-readable contract: `/dev-create-progress` parses the Implementation Phases section to build the project's phase index, so the phase headings **must** use the exact tag + attribute shape specified.
...
- **Implementation Phases (machine-readable — required shape).** Each phase in the Implementation Phases section MUST be an `<h3>` with this exact shape so `/dev-create-progress` can parse it:
  ```html
  ...
  - The visible text is `Phase <N>: <name>`. Do **not** put time estimates inside the heading text — put any estimate in a following `<p class="meta">`. `/dev-create-progress` uses the heading text after the colon, verbatim, as the phase name.
...
4. Suggest next steps (e.g., review, refinement, run `/dev-plan-phase` per phase)
...
- The PRD is consumed by `/dev-plan-phase` (reads scope/dependencies) and `/dev-create-progress` (parses the `data-phase` headings). Keeping the HTML contract intact is what lets that pipeline work unattended.
